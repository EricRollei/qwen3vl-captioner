...
class MainWindow(QMainWindow):
    ...
    def _on_caption_finished(self, caption: str):
        ...
        # ── Auto-save logic ──
        if self._batch_queue:
            # During batch: always auto-save silently, then continue
            self._auto_save_caption(self._current_image, caption)
            self._process_next_batch_item()
        elif self._settings_panel.auto_save_cb.isChecked():
            # Single image with auto-save enabled: save silently
            self._auto_save_caption(self._current_image, caption)
        else:
            # Single image, auto-save off: ask the user
